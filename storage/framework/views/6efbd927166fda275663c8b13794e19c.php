    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                <form method="GET" action="<?php echo e(route('dashboard.supervisor.order-assignment')); ?>" class="flex items-center gap-2">
                    <input
                        type="text"
                        name="search"
                    ...
                    <a href="<?php echo e(route('dashboard.supervisor.order-assignment')); ?>" class="inline-flex items-center px-4 py-2 rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-50 text-sm">مسح</a>
                </form>
            </div>
                    ...
                    <button type="button"
                        ...
                        <div class="flex justify-between items-start gap-2">
                            <div class="flex-1 min-w-0">
                                <div class="font-bold text-gray-900 text-base">#<?php echo e($order->order_number ?? $order->id); ?></div>
                                <div class="text-sm text-gray-600 mt-0.5"><?php echo e($order->recipient_name ?? optional($order->customer)->name ?? '—'); ?></div>
                                <div class="text-xs text-gray-500 mt-1"><?php echo e(optional($order->store)->name ?? '—'); ?></div>
                                <div class="text-xs text-gray-500 mt-1">
                                    <span class="font-semibold">Type:</span> <?php echo e($order->delivery_method ?? '—'); ?>

                                    <span class="mx-1">•</span>
                                    <span class="font-semibold">Date:</span> <?php echo e($order->estimated_delivery ? \Carbon\Carbon::parse($order->estimated_delivery)->format('Y-m-d') : '—'); ?>

                                </div>
                                <div class="text-sm font-semibold text-emerald-600 mt-2">$<?php echo e(number_format($order->total ?? $order->total_amount ?? 0, 2)); ?></div>
                            </div>
            ...
            <ul class="space-y-3">
                <?php $__currentLoopData = $availableDrivers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $driver): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li class="flex items-center justify-between p-3 rounded-xl bg-green-50 border border-green-100">
                        <div>
                            <div class="font-medium text-gray-900"><?php echo e(optional($driver->user)->name ?? optional($driver->user)->user_full_name ?? 'Driver #'.$driver->id); ?></div>
                            <div class="text-xs text-gray-500"><?php echo e(optional($driver->user)->phone ?? optional($driver->user)->mobile ?? '—'); ?></div>
                        </div>
                    ...
                    <li class="flex items-center justify-between p-3 rounded-xl border border-gray-200">
                        <div>
                            ...
                            <div class="text-xs text-gray-500">
                                <?php echo e(optional($assign->order)->delivery_method ?? '—'); ?>

                                <span class="mx-1">•</span>
                                <?php echo e(optional($assign->order)->estimated_delivery ? \Carbon\Carbon::parse(optional($assign->order)->estimated_delivery)->format('Y-m-d') : '—'); ?>

                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-blue-100 text-blue-800"><?php echo e($assign->status); ?></span>
                            <?php if(optional($assign->order)->status === 'out_for_delivery'): ?>
                                <form method="POST" action="<?php echo e(route('dashboard.supervisor.orders.change-status', $assign->order_id)); ?>">
                                    <?php echo csrf_field(); ?>
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <select id="driverSelect" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-700">
                            ...
                            <?php $__currentLoopData = $availableDrivers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $driver): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php $phone = optional($driver->user)->phone ?? optional($driver->user)->mobile ?? ''; ?>
                                <option value="<?php echo e($driver->id); ?>" data-phone="<?php echo e($phone); ?>"><?php echo e(optional($driver->user)->name ?? optional($driver->user)->user_full_name ?? 'Driver #'.$driver->id); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                    <div>
...
const orderDetailsUrl = '<?php echo e(url("/dashboard/supervisor/order-assignment/order")); ?>';
const assignUrl = '<?php echo e(route("dashboard.supervisor.assign-order")); ?>';
...
<?php echo $__env->make('dashboards.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Doaa\StudioProjects\Tulip-Store\resources\views/dashboards/supervisor/order-assignment.blade.php ENDPATH**/ ?>