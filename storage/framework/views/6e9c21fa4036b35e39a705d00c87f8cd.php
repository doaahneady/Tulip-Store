...
<div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
    <div class="flex items-center justify-between gap-3 mb-4">
        <a href="<?php echo e(route('dashboard.supervisor.drivers.create')); ?>" class="inline-flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-xl hover:bg-indigo-700 transition">
    ...
    <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <select name="availability" class="border rounded-xl px-4 py-2">
            ...
            <option value="offline" <?php if(request('availability')==='offline'): echo 'selected'; endif; ?>>غير متصل</option>
            <option value="on_break" <?php if(request('availability')==='on_break'): echo 'selected'; endif; ?>>في استراحة</option>
        </select>
                ...
                <tr>
                    ...
                    <td class="px-4 py-3">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                            ...
                            <?php echo e($driver->availability); ?>

                        </span>
                    </td>
                    <td class="px-4 py-3 text-right space-y-2">
                        <div class="flex flex-wrap items-center justify-end gap-2">
                            <a href="<?php echo e(route('dashboard.supervisor.drivers.edit', $driver)); ?>" class="inline-flex items-center gap-1 rounded-lg bg-indigo-600 text-white px-3 py-1.5 text-xs font-semibold hover:bg-indigo-700">
                                <i class="fas fa-pen"></i> تعديل البيانات
                            </a>
                            <form method="POST" action="<?php echo e(route('dashboard.supervisor.drivers.delete', $driver)); ?>" class="inline" onsubmit="return confirm('حذف هذا السائق نهائياً؟ لا يمكن التراجع.');">
                                <?php echo csrf_field(); ?>
                        ...
                        <form method="POST" action="<?php echo e(route('dashboard.supervisor.drivers.update-status', $driver)); ?>" class="flex flex-wrap items-center justify-end gap-2 border-t border-gray-100 pt-2">
                            <?php echo csrf_field(); ?>
                            <select name="status" class="border rounded-xl px-2 py-1 text-xs">
                                <option value="active" <?php if($driver->status==='active'): echo 'selected'; endif; ?>>نشط</option>
                                <option value="inactive" <?php if($driver->status==='inactive'): echo 'selected'; endif; ?>>غير نشط</option>
                                <option value="suspended" <?php if($driver->status==='suspended'): echo 'selected'; endif; ?>>موقوف</option>
                            </select>
                            <select name="availability" class="border rounded-xl px-2 py-1 text-xs">
                                <option value="available" <?php if($driver->availability==='available'): echo 'selected'; endif; ?>>متاح</option>
                                <option value="busy" <?php if($driver->availability==='busy'): echo 'selected'; endif; ?>>مشغول</option>
                                <option value="offline" <?php if($driver->availability==='offline'): echo 'selected'; endif; ?>>غير متصل</option>
                                <option value="on_break" <?php if($driver->availability==='on_break'): echo 'selected'; endif; ?>>في استراحة</option>
                            </select>
...
<?php echo $__env->make('dashboards.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\Tulip-Store\resources\views/dashboards/supervisor/drivers.blade.php ENDPATH**/ ?>