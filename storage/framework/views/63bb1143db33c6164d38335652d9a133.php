                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php $__empty_1 = true; $__currentLoopData = ($metrics['money_by_employee'] ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2.5 text-sm text-gray-800"><?php echo e(($employee->first_name ?? '') . ' ' . ($employee->last_name ?? '')); ?></td>
                                <td class="px-4 py-2.5 text-sm font-semibold text-gray-900"><?php echo app(App\Services\CurrencyService::class)->formatUsd((float)($employee->total_pay ?? 0)); ?></td>
                            </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <tr><td colspan="2" class="px-4 py-6 text-center text-gray-500">لا توجد بيانات</td></tr>
                        <?php endif; ?>
...
<?php $component->withAttributes(['title' => 'طلبات قيد الانتظار','icon' => 'fas fa-hourglass-half','subtitle' => 'الموافقات والتحويلات']); ?>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
        <div class="p-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('dashboard.finance.approvals')); ?>" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800">عرض الكل</a>
        </div>
        <div class="p-4">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php $__empty_1 = true; $__currentLoopData = ($metrics['pending_approvals_list'] ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2.5 text-sm text-gray-800"><?php echo e($t->type); ?></td>
                                <td class="px-4 py-2.5 text-sm font-semibold text-gray-900"><?php echo app(App\Services\CurrencyService::class)->formatUsd((float)($t->amount ?? 0)); ?></td>
                                <td class="px-4 py-2.5 text-sm text-gray-700"><?php echo e(optional($t->order)->order_number ?? '-'); ?></td>
                                <td class="px-4 py-2.5 text-sm text-gray-500"><?php echo e($t->created_at?->diffForHumans()); ?></td>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
        <div class="p-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('dashboard.finance.payouts')); ?>" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800">عرض الكل</a>
        </div>
        <div class="p-4">
            <div class="overflow-auto max-h-[360px]">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php $__empty_1 = true; $__currentLoopData = ($metrics['pending_payouts_list'] ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2.5 text-sm text-gray-800"><?php echo e(optional($p->store)->name ?? ('Store #'.$p->store_id)); ?></td>
                                <td class="px-4 py-2.5 text-sm font-semibold text-gray-900"><?php echo app(App\Services\CurrencyService::class)->formatUsd((float)($p->amount ?? 0)); ?></td>
                                <td class="px-4 py-2.5 text-sm text-gray-500"><?php echo e($p->created_at?->diffForHumans()); ?></td>
                                <td class="px-4 py-2.5">
                                    <div class="flex items-center gap-2">
...
<?php echo $__env->make('dashboards.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\Tulip-Store\resources\views/dashboards/finance/index.blade.php ENDPATH**/ ?>