...
function initializeModernSlider() {
    ...
    sliderData.forEach((slide, index) => {
        ...
        dot.style.cssText = 'width:12px; height:12px; border-radius:50%; border:2px solid #2a7080; background:transparent; cursor:pointer; transition:all 0.3s;';
...
function scrollCategoriesLeft() {
    document.getElementById('categoriesScroll').scrollBy({left: -250, behavior: 'smooth'});
}
...
function getCategoryIcon(categoryName) {
    ...
    if (nameLower.includes('زهور') || nameLower.includes('زهر') || nameLower.includes('طازة')) {
        return 'fa-seedling';
    }
    if (nameLower.includes('ورد') || nameLower.includes('سلل')) {
        return 'fa-heart';
    }
    if (nameLower.includes('هدايا') || nameLower.includes('هدية') || nameLower.includes('مفاج')) {
        return 'fa-gift';
    }
    if (nameLower.includes('أطفال') || nameLower.includes('اطفال') || nameLower.includes('طفل')) {
        return 'fa-baby';
    }
...
function animateCartIcon() {
    const cartIcon = document.querySelector('.cart-icon') || document.querySelector('[href="/cart"]') || document.querySelector('.navbar-cart');
    ...
    const cartBadge = document.querySelector('.cart-badge') || document.querySelector('.cart-count');
    if (cartBadge) {
...
async function addToCart(productId, button) {
    ...
    try {
        ...
        if (data.success) {
            ...
            button.innerHTML = '<i class="fas fa-check" style="font-size:1.4rem;"></i>';
