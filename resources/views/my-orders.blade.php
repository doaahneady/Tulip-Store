...
<tr>
<td><strong style="color:#ff6b35">{{ $order->order_number }}</strong></td>
<td>{{ $order->created_at->format('d/m/Y') }}<br><small style="color:#999">{{ $order->created_at->format('h:i A') }}</small></td>
<td>
...
<span style="font-weight:600;color:#2a7080">{{ \Carbon\Carbon::parse($order->estimated_delivery)->format('d/m/Y') }}</span>
@php
