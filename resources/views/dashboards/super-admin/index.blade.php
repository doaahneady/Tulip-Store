...
<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    <div class="flex flex-wrap items-center gap-2">
        ...
        <a href="{{ route('dashboard.admin.users') }}" class="inline-flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-xl hover:bg-indigo-700 transition">
        ...
        <a href="{{ route('dashboard.admin.inventory.alerts') }}" class="inline-flex items-center gap-2 bg-amber-600 text-white px-4 py-2 rounded-xl hover:bg-amber-700 transition">
        ...
        <a href="{{ route('dashboard.hr.skills') }}" class="inline-flex items-center gap-2 bg-violet-600 text-white px-4 py-2 rounded-xl hover:bg-violet-700 transition">
            <i class="fas fa-graduation-cap"></i>
            <span>مهارات الموظفين</span>
        </a>
        <a href="{{ route('dashboard.my-attendance.index') }}" class="inline-flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-xl hover:bg-emerald-700 transition">
            <i class="fas fa-user-clock"></i>
            <span>حضوري</span>
        </a>
    ...
    <a href="{{ route('dashboard.admin.cross-department-kpis') }}" class="inline-flex items-center gap-2 bg-purple-600 text-white px-4 py-2 rounded-xl hover:bg-purple-700 transition">
    ...
    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 xl:col-span-4">
        ...
        <div class="overflow-auto max-h-[320px]">
            <table class="w-full text-sm">
                ...
                <tbody>
                    @forelse(($metrics['top_products_30d'] ?? []) as $row)
                        <tr class="border-t border-gray-100">
                            <td class="py-2 text-gray-800">{{ $row->name ?? 'منتج' }}</td>
                            <td class="py-2 text-gray-700">{{ number_format((int) ($row->total_sold ?? 0)) }}</td>
                            <td class="py-2 text-gray-700">{{ number_format((float) ($row->total_revenue ?? 0), 2) }}</td>
                        </tr>
                    @empty
            ...
            @forelse(($metrics['recent_activities'] ?? []) as $activity)
                <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                    <div class="w-9 h-9 rounded-xl bg-white border border-gray-100 flex items-center justify-center">
                        <i class="fas {{ $activity['icon'] ?? 'fa-bolt' }} {{ $activity['color'] ?? 'text-gray-700' }}"></i>
                    </div>
                    <div class="flex-1">
                        <p class="text-sm font-semibold text-gray-900">{{ $activity['title'] ?? 'Activity' }}</p>
                        <p class="text-xs text-gray-600">{{ $activity['description'] ?? '' }}</p>
                        <p class="text-xs text-gray-400 mt-1">{{ optional($activity['time'] ?? null)->diffForHumans() ?? '' }}</p>
                    </div>
                    @if(isset($activity['amount']) && $activity['amount'] !== null)
                        <div class="text-xs font-semibold text-gray-700">{{ number_format((float) $activity['amount'], 2) }}</div>
                    @endif
                </div>
            @empty
            ...
            <div class="space-y-2 max-h-28 overflow-y-auto">
                @forelse(($metrics['low_stock_products'] ?? []) as $p)
                    <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg text-sm">
                        ...
                        <span class="text-xs px-2 py-1 rounded bg-amber-100 text-amber-700">{{ (int) ($p->stock_quantity ?? 0) }}</span>
                    </div>
                @empty
    ...
    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100" role="region" aria-labelledby="growthHeading">
        ...
        <div class="grid grid-cols-3 gap-3">
            <div class="p-4 rounded-2xl text-center border border-gray-600 bg-blue-800" aria-label="نمو المستخدمين">
                <p class="text-2xl font-extrabold text-white leading-none"><span dir="ltr">{{ number_format($metrics['user_growth'] ?? 0, 1) }}%</span></p>
            ...
            <div class="p-4 rounded-2xl text-center border border-gray-600 bg-emerald-800" aria-label="نمو الإيرادات">
                <p class="text-2xl font-extrabold text-white leading-none"><span dir="ltr">{{ number_format($metrics['revenue_growth'] ?? 0, 1) }}%</span></p>
            ...
            <div class="p-4 rounded-2xl text-center border border-gray-600 bg-orange-800" aria-label="نمو الطلبات">
                <p class="text-2xl font-extrabold text-white leading-none"><span dir="ltr">{{ number_format($metrics['order_growth'] ?? 0, 1) }}%</span></p>
    ...
    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        ...
        <div class="grid grid-cols-2 gap-3">
            <div class="p-3 bg-red-800 rounded-xl text-center border border-gray-600">
                <p class="text-2xl font-bold text-white">{{ number_format($metrics['system_alerts'] ?? 0) }}</p>
                <p class="text-xs text-red-200">تنبيهات النظام</p>
            </div>
            <div class="p-3 bg-purple-800 rounded-xl text-center border border-gray-600">
                <p class="text-2xl font-bold text-white">{{ number_format($metrics['avg_order_value'] ?? 0, 2) }}</p>
        ...
        <div class="space-y-2 max-h-[280px] overflow-y-auto">
            @forelse(($metrics['top_performing_stores'] ?? []) as $store)
                <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg text-sm">
                    ...
                    <span class="text-gray-700">{{ $storeName }}</span>
                    <span class="px-2 py-1 text-xs rounded bg-indigo-100 text-indigo-700">{{ number_format($store->orders_sum_total ?? 0, 2) }}</span>
                </div>
            @empty
...
<script>
    ...
    const geoPoints = @json($geoPoints);
    const metricsUrl = @json(route('dashboard.admin.metrics'));
