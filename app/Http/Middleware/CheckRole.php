...
class CheckRole
{
    ...
    {
        if (! auth()->check() && ! auth('employee')->check() && ! auth('trader')->check()) {
        ...
        $user = auth('employee')->user() ?? auth('trader')->user() ?? auth()->user();
        ...
        foreach ($roles as $role) {
            ...
            switch ($role) {
                ...
                case 'accountant':
                case 'finance':
                    if ($user->is_finance ?? false || $user->is_accountant ?? false || $user->is_admin) {
                        return $next($request);
                    }
                    break;
